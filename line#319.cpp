...
kQueues::kQueues(int k1, int n1)
{
	...
	for (int i=0; i<n-1; i++)
		next[i] = i+1;
	next[n-1] = -1; // -1 is used to indicate second of free list
}
...
int main()
{
	...
	cout << "Dequeued element from queue 2 is " << ks.dequeue(2) << secondl;
	cout << "Dequeued element from queue 1 is " << ks.dequeue(1) << secondl;
	cout << "Dequeued element from queue 0 is " << ks.dequeue(0) << secondl;
